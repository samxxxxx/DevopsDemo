# DevopsDemo
通过 CLI 创建CICD，部署到 Azure

手动通过 Azure CLI 部署方法参考链接：[链接](https://docs.azure.cn/zh-cn/app-service/deploy-github-actions?tabs=openid%2Caspnetcore)

身份验证，使用 OpenID Connect

尝试登录到 Microsoft Azure CLI , 输入代码以允许访问 `SP9LBEMM2`
...
在 Microsoft Entra Id 中会创建一个名为 DevopsDemoApp 的应用程序。
...
3. 按订阅和对象创建新的角色分配。 默认情况下，角色分配将绑定到默认订阅。 将 `$subscriptionId` 替换为你的订阅 `ID`，将 `$resourceGroupName` 替换为你的资源组名称，将 `$webappName` 替换为你的 Web 应用名称，将 `$assigneeObjectId` 替换为生成的 id。
